...
function User (params) {
  ...
  this.scoreUser = async (params) => {
    ...
    opts.uri = this.defaultOpts.uri + this.username
    ...
    try {
      ...
      //console.log(this.username + 'ATTRS: ' + attrs + ' EVENTS: ' + events + ' REC_EVENTS: ' + receivedEvents + ' TOTAL SCORE: ' + total)
  ...
  this.realUser = async () => {
    ...
    return {pass: pass, score: score, user: this.username}
  }

  this.topOrg = async () => {
    try {
      let response = await this.octokit.orgs.getForUser({username: this.username, per_page: 100 })
  ...
  this.topRepo = async () => {
    try { 
      let response = await this.octokit.repos.getForUser({username: this.username })
      let data = response.data
      while (this.octokit.hasNextPage(response)) {
        if (data.length > 50) { break }
        response = await this.octokit.getNextPage(response)
  ...
  this.eventsScore = async () => {
    var score = 0
    var opts = Object.assign({}, this.defaultOpts)
    opts.uri = 'https://api.github.com/users/' + this.username + '/events'
 ...
 */
  ...
  this.receivedEventsScore = async () => {
    var score = 0
    var opts = Object.assign({}, this.defaultOpts)
    opts.uri = 'https://api.github.com/users/' + this.username + '/received_events'
