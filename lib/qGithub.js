  ...
  this.run = async () => {
    try {
      ...
      if(topOwnedRepo) { 
        ...
        console.log(topOwnedRepo.full_name)
      }
      if(topPushedRepo) {
        pushedRepoScore = await this.qRepo.scoreRepo(topPushedRepo)
        console.log(topPushedRepo)
      }
      if(topOrg) {
      ...
      console.log(this.username + " USER:" + userScore + " REPO:"  + ownedRepoScore + " ORG:" + ":" + orgScore + " PUSHED: " + pushedRepoScore + " FOLLOWER:" + followerScore)

      let foss = Math.max(ownedRepoScore, pushedRepoScore, orgScore)
      return { real: userScore, repo: ownedRepoScore, pushed_repo: pushedRepoScore, org: orgScore, foss: foss}
